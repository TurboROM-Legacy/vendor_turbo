...
function turbo_rename_function()
{
    eval "original_turbo_$(declare -f ${1})"
}

function _turbo_build_hmm() #hidden
{
    printf "%-8s %s" "${1}:" "${2}"
}

function turbo_append_hmm()
{
    HMM_DESCRIPTIVE=("${HMM_DESCRIPTIVE[@]}" "$(_turbo_build_hmm "$1" "$2")")
}

function turbo_add_hmm_entry()
{
    for c in ${!HMM_DESCRIPTIVE[*]}
    do
        if [[ "${1}" == $(echo "${HMM_DESCRIPTIVE[$c]}" | cut -f1 -d":") ]]
        then
            HMM_DESCRIPTIVE[${c}]="$(_turbo_build_hmm "$1" "$2")"
            return
        fi
    done
    turbo_append_hmm "$1" "$2"
}

function turboremote()
{
    ...
    git remote rm turbo 2> /dev/null
    ...
    git remote add turbo "git@github.com:AOSPB/$pfx$project"
...
function hmm() #hidden
{
    ...
    echo "vendor/turbo extended functions. The complete list is:"
    for i in $(grep -P '^function .*$' "$T/vendor/turbo/build/envsetup.sh" | grep -v "#hidden" | sed 's/function \([a-z_]*\).*/\1/' | sort | uniq); do
        echo "$i"
    done |column
}

turbo_append_hmm "turboremote" "Add a git remote for matching AOSPB repository"
